...
//        menu.addItem(withTitle: tr("macMenuAddEmptyTunnel"), action: #selector(handleAddEmptyTunnelAction), keyEquivalent: "n")
    ...
    let actionButton: NSPopUpButton = {
        ...
        menu.addItem(withTitle: tr("macMenuChangeLanguage"), action: #selector(changeAppLanguage), keyEquivalent: "")
        menu.autoenablesItems = false
    ...
    @objc func handleExportTunnelsAction() {
        PrivateDataConfirmation.confirmAccess(to: tr("macExportPrivateData")) { [weak self] in
            ...
            savePanel.nameFieldStringValue = "PGate-export.zip"
...
//                if code.contains("ja") {
        ...
        let alert = NSAlert()
        alert.messageText = tr("Alert")
        alert.informativeText = tr("macMenuQuitpGate")
        alert.beginSheetModal(for: self.view.window!, completionHandler: { (modalResponse) -> Void in
            for runningApplication in NSWorkspace.shared.runningApplications{
                if(runningApplication.bundleIdentifier == Optional("com.yc.pgate")){
